...
main = do
  ...
  -- graph系の問題でよく使うやつ
  let n = 4
  let graph = accumArray (flip (:)) [] (1, n) $ concatMap (\[a, b] -> [(a, b), (b, a)]) [[1, 3], [2, 3], [2, 4]]
  let graph2 = accumArray (flip (:)) [] (1, n) $ concatMap (\[a, b] -> [(a, b), (b, a)]) [[1, 3], [2, 3]] -- index4が登場しない場合 array (1,4) [(1,[3]),(2,[3]),(3,[2,1]),(4,[])]
  putStrLn "--graph--"
  print graph
  print graph2
  print $ assocs graph -- [(1,[3]),(2,[4,3]),(3,[2,1]),(4,[2])]
