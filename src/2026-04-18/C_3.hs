  ...
  where
    ...
    bfs visited (x : xs) =
      let neighbors = IM.findWithDefault [] x graph -- xから獲得できるアイテム
          newNodes = filter (`IS.notMember` visited) neighbors
          -- visited' = foldr IS.insert visited newNodes
          -- NOTE: visitedとnewNodesは同じループのなかで更新できるがやっていないのでTLEしているよう。
          visited' = foldl' (flip IS.insert) visited newNodes
