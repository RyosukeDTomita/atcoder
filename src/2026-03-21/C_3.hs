...
getWhiteIndex h w grid =
  [ (i, j)
    | i <- [0 .. h - 1],
      j <- [0 .. w - 1],
      BS.index (grid V.! i) j == '.' -- [BS.ByteString]だとgrid !! iがO(i)で遅いのでVectorにした。
  ]
...
main = BS.interact $ \inputs ->
  let ls = BS.lines inputs
      ...
      grid = V.fromList (drop 1 ls) -- O(1)アクセスのVectorに変換
